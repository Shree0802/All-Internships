...
# NLTK corpora are optional at inference time.
# If a serverless environment does not have them, the cleaner uses safe fallbacks.
        ...
        if token not in stop_words and len(token) > 1:
            ...
            if apply_lemmatization:
                try:
                    ...
                    # Vercel/serverless environments may not have the WordNet corpus.
                    # Continue with stemming so inference does not depend on runtime downloads.
                    pass
            if apply_stemming:
