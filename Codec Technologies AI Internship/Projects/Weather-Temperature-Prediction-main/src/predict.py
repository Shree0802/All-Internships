"""Inference engine for the Weather Temperature Prediction project."""
...
class TemperaturePredictor:
    """Load the trained weather model and perform predictions."""
    ...
    def load_artifacts(self):
        ...
        if not best_model_path.exists() or not metadata_path.exists():
            raise FileNotFoundError(
                "ML model files are missing. Include models/best_model.pkl and models/metadata.json."
            )
    ...
    def predict(
        ...
        predicted_temp = float(np.round(self.model.predict(X_scaled)[0], 2))

        rmse = self.metadata.get("RMSE", 0.5)
        margin = 1.96 * max(rmse, 0.2)
        elapsed_ms = round((time.time() - start_time) * 1000, 2)

        return {
            "predicted_temperature": predicted_temp,
            "confidence_lower": float(np.round(predicted_temp - margin, 2)),
            "confidence_upper": float(np.round(predicted_temp + margin, 2)),
    ...
    def predict_future_trend(self, n_days: int = 14) -> pd.DataFrame:
        ...
        if not processed_path.exists():
            raise FileNotFoundError(
                "Processed weather data is missing. Include data/processed/weather_data_processed.csv."
            )
        ...
        for f_date in future_dates:
            ...
            predictions.append({
                ...
                "Simulated_Humidity": round(float(sim_humidity), 1),
                "Simulated_Pressure": round(float(sim_pressure), 1),
            })
            curr_temp = pred_temp
